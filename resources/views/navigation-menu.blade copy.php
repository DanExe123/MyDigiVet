        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-10 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link href="{{ route('ClientCommunication.index') }}" :active="request()->routeIs('ClientCommunication.index')" class="custom-bg-357D7F text-cyan-50">
                    ...
                    <div class="dropdown dropdown-hover mt-5">
                        ...
                        <ul class="menu dropdown-content custom-bg-357D7F rounded-box z-[1] w-full p-2 shadow">
                            <li><a href="{{ route('Listofappointment.index') }}" class="{{ request()->routeIs('Listofappointment.index') ? 'bg-cyan-700 text-cyan-50' : 'text-cyan-500' }} rounded px-4 py-2">My Appointment</a></li>
                          </ul>
                    </div>
                    <div class="dropdown dropdown-hover mt-5">
                    <x-nav-link href="{{ route('MyPets.index') }}" :active="request()->routeIs('MyPets.index')" class="custom-bg-357D7F text-cyan-50">
                        ...
                        <ul class="menu dropdown-content custom-bg-357D7F rounded-box z-[1] w-62 p-2 shadow">
                             <li><a href="{{ route('Treatment.index') }}" class="{{ request()->routeIs('Treatment.index') ? 'bg-cyan-700 text-cyan-50' : 'text-cyan-500' }} rounded px-4 py-2">Treatment</a></li>
                            <li><a href="{{ route('Diet.index') }}" class="{{ request()->routeIs('Diet.index') ? 'bg-cyan-700 text-cyan-50' : 'text-cyan-500' }} rounded px-4 py-2">Diet</a></li>
                            <li><a href="{{ route('Prescription.index') }}" class="{{ request()->routeIs('Prescription.index') ? 'bg-cyan-700 text-cyan-50' : 'text-cyan-500' }} rounded px-4 py-2">Prescription</a></li>
                        </ul>
                    </div>
                        ...
                        <x-slot name="trigger">
                            @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                <button class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition">
                                    <img class="h-24 w-24 rounded-full object-cover" src="{{ asset('storage/' . Auth::user()->profile_photo_path) }}" alt="{{ Auth::user()->name }}'s Profile Picture">
                                </button>
                            @else
                                <span class="inline-flex rounded-md">
...
<!-- This is old section of navigation bar

                        <x-slot name="trigger">
                            @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                <button class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition">
                                    <img class="h-24 w-24 rounded-full object-cover" src="{{ asset('storage/' . $profile_photo_path) }}" alt="{{ $name }}'s Profile Picture">
                                 <!--   <img class="h-8 w-8 rounded-full object-cover" src="{{ Auth::user()->profile_photo_path }}" alt="{{ Auth::user()->name }}" /0> 
                                </button>
                            @else
                                <span class="inline-flex rounded-md">
                                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-black dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none focus:bg-gray-50 dark:focus:bg-gray-700 active:bg-gray-50 dark:active:bg-gray-700 transition ease-in-out duration-150">
